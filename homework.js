...
1. HTTP 狀態碼的分類（1xx, 2xx, 3xx, 4xx, 5xx 各代表什麼）
   答：
	 1xx：資訊回應，表示請求已收到，伺服器正在繼續處理。
	 2xx：成功，表示請求已成功被伺服器接收、理解並處理。
	 3xx：重新導向，表示資源位置有變化，或需要額外動作才能完成請求。
	 4xx：用戶端錯誤，通常是請求格式錯誤、參數錯誤、權限不足或找不到資源。
	 5xx：伺服器錯誤，表示伺服器在處理請求時發生問題。

2. GET、POST、PATCH、PUT、DELETE 的差異
   答：
	 GET：取得資料，用來查詢資源
	 POST：新增資料
	 PATCH：部分更新，只修改資源中的部分欄位
	 PUT：全部更新，會覆蓋整個資源 (就算沒有修改的欄位也會被覆蓋掉)
	 DELETE：刪除資料

3. 什麼是 RESTful API？
   答：
	 基於 HTTP 協定、符合 REST 架構風格的網路應用程式介面。它利用標準 HTTP 方法（GET, POST, PUT, DELETE）操作以 URL 定義的資源，並且通常使用 JSON 格式傳輸資料。
